...
namespace :deploy do
  ...
  task :setup_config, roles: :app do
    sudo "ln -nfs #{current_path}/config/nginx_#{fetch(:stage)}.conf /etc/nginx/sites-enabled/#{application}_#{fetch(:stage)}"
    sudo "ln -nfs #{current_path}/config/unicorn_init_#{fetch(:stage)}.sh /etc/init.d/unicorn_#{application}_#{fetch(:stage)}"
